...
public class TuningTrouble {
    ...
    private static final int MAX_MARKER_SIZE = 14;
    public int[] numberMessageCharactersNeededList(List<String> testList) {
        int[] output = new int[testList.size()];
        for (int i = 0; i < testList.size(); i++) {
            output[i] = numberMessageCharactersNeeded(testList.get(i));
        }
        return output;
    }

    private int numberMessageCharactersNeeded(String streamLine) {
